...
export function deriveOfflineMatchHud(state) {
  return deriveHudData(state.matchState, { localSlotIndex: LOCAL_PLAYER_SLOT_INDEX });
}
...
export function advanceOfflineMatchTick(state, { localInput = null, blockers = [] } = {}) {
  ...
  let workingState = localInput ? applyOfflineLocalInput(state, localInput) : state;
  ...
  return Object.freeze({ ...workingState, matchState, metrics });
}
